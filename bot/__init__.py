...
from .main import is_disabled, error_handler, Context
...
async def create_pool(client, loop) -> asyncpg.Pool:
...
async def get_prefix(client, message) -> Union[list, str]:
    default_prefix:str = botdata["prefix"]
...
class Bot(BotBase):
    ...
    async def on_message_delete(self, message:discord.Message) -> None:
        if message.author.bot:
            pass
        try:
            bot_response:discord.Message = self.old_responses[message.id]
